...
while {true} do {
    if ([5] call KPLIB_fnc_hasPermission) then {
        ...
        {
            if ((captive _x) && !(_x in _actionned_captive_units) && !((side group _x) == KPLIB_side_friendly) && !(_x getVariable ["ACE_isUnconscious", false])) then {
                _x addAction ["<t color='#FFFF00'>" + localize "STR_SECONDARY_CAPTURE" + "</t>",{[_this select 0] join (group player);},"",-850,true,true,"","(vehicle player == player) && (side group _target != KPLIB_side_friendly) && (captive _target)"];
                _actionned_captive_units pushback _x;
            };
        } forEach _near_people;

        {
            if (!(alive _x) || ((player distance _x) > 5) || ((side group _x) == KPLIB_side_friendly)) then {
                removeAllActions _x;
