...
if ((toLower _type) in KPLIB_o_allVeh_classes) then {
    if (_vehToRecycle isKindOf "Car") then {
        _price_s = round (300 * _suppMulti);
        _price_a = round (100 * _ammoMulti);
        _price_f = round (300 * _fuelMulti);
    };
    if (_vehToRecycle isKindOf "Tank") then {
        _price_s = round (800 * _suppMulti);
        _price_a = round (800 * _ammoMulti);
        _price_f = round (800 * _fuelMulti);
    };
    if (_vehToRecycle isKindOf "Air") then {
        _price_s = round (1000 * _suppMulti);
        _price_a = round (1000 * _ammoMulti);
        _price_f = round (1000 * _fuelMulti);
    };
} else {
