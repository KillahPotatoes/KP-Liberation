...
if ( KPLIB_halo_param > 1 && ( KPLIB_last_halo_jump + ( KPLIB_halo_param * 60 ) ) >= time ) exitWith {
    hint format [ localize "STR_HALO_DENIED_COOLDOWN", ceil ( ( ( KPLIB_last_halo_jump + ( KPLIB_halo_param * 60 ) ) - time ) / 60 ) ];
};
...
if ( dojump > 0 ) then {
    ...
    halo_position = [ halo_position select 0, halo_position select 1, KPLIB_halo_altitude + (random 200) ];
    halojumping = true;
    sleep 0.1;
