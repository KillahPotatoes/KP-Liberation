...
if !(_spawn_marker isEqualTo "") then {
    ...
    if (_combat_readiness < 60) then {_target_size = round (_target_size * 0.65);};
    ...
    if (_infOnly) then {
        ...
        if (GRLIB_csat_aggressivity > 0.9) then {
            [[markerPos _spawn_marker] call KPLIB_fnc_getNearestBluforObjective, _combat_readiness] spawn spawn_air;
        };
    };
};
