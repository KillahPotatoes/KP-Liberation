    ...
    if ([5] call KPLIB_fnc_hasPermission) then {
...

        {
            ...
            if (!(_next_truck in _managed_trucks) && (_truck_load > 0)) then {
                    ...
                    _next_truck setVariable [ "KPLIB_ammo_truck_action", _action_id, false ];
                    _managed_trucks pushback _next_truck;
            };

            if ((_next_truck in _managed_trucks) && _truck_load == 0) then {
                _next_truck removeAction (_next_truck getVariable ["KPLIB_ammo_truck_action", -1]);
...

        {
            ...
            if (!(_next_box in _managed_boxes) && ( isNull  attachedTo _next_box )) then {
                ...
                _b_action_id5 = _next_box addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_CRATE_CARRY" + "</t>",{(_this select 0) attachTo [(_this select 1), [0, 2, 1]];},"",-504,true,false,"","build_confirmed == 0 && (_this distance _target < 5) && (vehicle player == player) && (isNull attachedTo _target)"];
...

        {
            ...
            if (!(_next_box in _managed_boxes) || !( isNull  attachedTo _next_box )) then {
                ...
                _next_box removeAction (_next_box getVariable ["KPLIB_ammo_box_action", -1]);
