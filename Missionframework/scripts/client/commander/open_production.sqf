private ["_dialog", "_fontSize", "_color_positive", "_color_neutral", "_color_negative", "_color_actual", "_sectorType", "_storage", "_crateCount", "_crateMax", "_storagespace", "_productiontime", "_saveChanges", "_listselect", "_selectedSector", "_mapdisplay", "_supplyValue", "_ammoValue", "_fuelValue"];
...
_mapdisplay = ((findDisplay 758001) displayCtrl 7580025);
...
while {dialog && (alive player)} do {
	...
	if (saveSectorSetting == 1) then {
		saveSectorSetting = 0;
		hint localize "STR_PRODUCTION_SAVED";
	};

	if ((lbCurSel 7580012) != _listselect) then {
		...
		if ((count (_selectedSector select 3)) > 0) then {
			...
			((findDisplay 758001) displayCtrl 7580015) ctrlSetText _storagespace;
			((findDisplay 758001) displayCtrl 7580015) ctrlSetTextColor _color_actual;
			((findDisplay 758001) displayCtrl 7580016) ctrlSetText _productiontime;
			((findDisplay 758001) displayCtrl 7580016) ctrlSetTextColor _color_neutral;
			_color_actual = _color_neutral;
		} else {
		...
		switch (_selectedSector select 2) do {
			case 1: {
				((findDisplay 758001) displayCtrl 7580017) ctrlSetTextColor _color_positive;
			...
			default {
				...
				((findDisplay 758001) displayCtrl 7580017) ctrlSetTextColor _color_actual;
		...
		((findDisplay 758001) displayCtrl 7580020) ctrlSetText _supplyValue;
		((findDisplay 758001) displayCtrl 7580021) ctrlSetText _ammoValue;
		((findDisplay 758001) displayCtrl 7580022) ctrlSetText _fuelValue;
	...
	waitUntil {!dialog || !(alive player) || (lbCurSel 7580012) != _listselect || saveSectorSetting != 0 || saveChanges != 0};
};
