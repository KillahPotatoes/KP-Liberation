...
for [{_i=0}, {!isNil ("boat_" + str _i)}, {_i = _i + 1}] do {
	...
	_KP_liberation_boat = KP_liberation_boat_classname createVehicle [((getposATL _KP_liberation_boat_spawn) select 0),((getposATL _KP_liberation_boat_spawn) select 1),((getposATL _KP_liberation_boat_spawn) select 2) + 0.2];
...
if (KP_liberation_debug) then {private _text = format ["[KP LIBERATION] [DEBUG] Startvehicle spawn done by: %1", (name player)];_text remoteExec ["diag_log",2];};
