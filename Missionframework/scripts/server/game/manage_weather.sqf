...
] select (KPLIB_weather_param - 1);

private _newWeather = selectRandom _weathers;
0 setOvercast _newWeather;
forceWeatherChange;

[format ["Set initial weather to: %1 - Param Value: %2 - Time: %3", _newWeather, KPLIB_weather_param, diag_tickTime], "WEATHER"] call KPLIB_fnc_log;
