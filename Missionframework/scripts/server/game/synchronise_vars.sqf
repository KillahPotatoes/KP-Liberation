...
if (KP_liberation_debug) then {private _text = format ["[KP LIBERATION] [DEBUG] Serverside sync variables: KP_liberation_production"];_text remoteExec ["diag_log",2];};
waitUntil{!isNil "KP_liberation_logistics"};
if (KP_liberation_debug) then {private _text = format ["[KP LIBERATION] [DEBUG] Serverside sync variables: KP_liberation_logistics"];_text remoteExec ["diag_log",2];};
