...
while { KPLIB_cleanup_vehicles > 0 } do {
...

    {
        ...
        if ( count _nearestfob == 3 ) then {
            if ( ( _nextvehicle distance _nearestfob > ( 1.2 * KPLIB_fob_range ) ) && ( _nextvehicle distance startbase > ( 1.2 * KPLIB_fob_range ) ) ) then {
        ...
        if (  _nextvehicle getVariable [ "KPLIB_empty_vehicle_ticker", 0 ] >= ( 6 * KPLIB_cleanup_vehicles ) ) then {
