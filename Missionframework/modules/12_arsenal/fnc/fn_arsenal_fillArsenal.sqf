...
if (KPLIB_ace_enabled) then {
    [KPLIB_preset_arsenal_whitelist ,{[player, true, false] call ace_arsenal_fnc_removeVirtualItems}] remoteExec ["call", [-2, 0] select hasInterface, true];
};
...
if !(_goodItems isEqualTo true) then {
    ...
    if (KPLIB_ace_enabled) then {
        [_goodItems ,{[player, _this, false] call ace_arsenal_fnc_addVirtualItems}] remoteExec ["call", [-2, 0] select hasInterface, true];
    };
} else {
    // Fill the arsenal
