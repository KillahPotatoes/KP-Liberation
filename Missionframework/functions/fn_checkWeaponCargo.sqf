/*
    ...
    Description:
        Removes weapons not found in KPLIB_allowed_items from a cargo space.
...

{
    ...
    if ((toLower ([_x] call BIS_fnc_baseWeapon)) in KPLIB_allowed_items) then {
        ...
        if !(toLower _muzzle in _presetAttachments) then {
            if (toLower _muzzle in KPLIB_allowed_items) then {
        ...
        if !(toLower _pointer in _presetAttachments) then {
            if (toLower _pointer in KPLIB_allowed_items) then {
        ...
        if !(toLower _optic in _presetAttachments) then {
            if (toLower _optic in KPLIB_allowed_items) then {
        ...
        if !(toLower _bipod in _presetAttachments) then {
            if (toLower _bipod in KPLIB_allowed_items) then {
        ...
        if (_magazineClass != "") then {
            if (toLower _magazineClass in KPLIB_allowed_items) then {
        ...
        if (_magazineGLClass != "") then {
            if (toLower _magazineGLClass in KPLIB_allowed_items) then {
