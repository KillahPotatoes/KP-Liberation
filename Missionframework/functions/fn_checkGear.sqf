...
if !(toLower (headgear player) in KPLIB_allowed_items) then {
    _removedItems pushBack (headgear player);
    removeHeadgear player;
};
if !(toLower (goggles player) in KPLIB_allowed_items) then {
    _removedItems pushBack (goggles player);
    removeGoggles player;
};
if !(toLower (uniform player) in KPLIB_allowed_items) then {
    _removedItems pushBack (uniform player);
    removeUniform player;
};
if !(toLower (vest player) in KPLIB_allowed_items) then {
    _removedItems pushBack (vest player);
    removeVest player;
};
if (!(toLower (backpack player) in KPLIB_allowed_items) && ((backpack player) != _backpack)) then {
...
} forEach (((_items arrayIntersect _items) - KPLIB_allowed_items) select {!([_x] call KPLIB_fnc_isRadio)});
...
} forEach ((_items arrayIntersect _items) - KPLIB_allowed_items);
...
} forEach (_items - KPLIB_allowed_items);
